...
EVRInitError ControllerDriver::Activate(uint32_t unObjectId)
{
	...
	VRProperties()->SetStringProperty(props, Prop_InputProfilePath_String, "{infinity}/input/controller_profile.json"); //tell OpenVR where to get your driver's Input Profile
...
{
	// pipe from Infinity Play 
	fileHandle = CreateFileW(TEXT("\\\\.\\pipe\\infinity-pipe"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	...
	{
		_steamSpeed = (_bgSpeed * 0.95f) / 50.0f; // scale BG speed to joystick mapping
	}

	//Since we used VRScalarUnits_NormalizedTwoSided as the unit, the range is -1 to 1.
	VRDriverInput()->UpdateScalarComponent(joystickYHandle, _steamSpeed, 0); //move forward
	VRDriverInput()->UpdateScalarComponent(trackpadYHandle, _steamSpeed, 0); //move foward
...
int ControllerDriver::ReadString(char* output) {
	ULONG read = 0;
	int index = 0;
	try {
		do {
